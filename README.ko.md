...
Claude Code는 세션이 바뀌면 기억이 리셋됩니다. 어제 `--no-verify` 때문에 파이프라인이 터졌던 것도, async 핸들러에 특정 패턴을 써야 했던 것도 까먹습니다. 결국 같은 걸 또 알려주고, 또 고치고, 토큰은 토큰대로 날립니다.

## Forge는

코딩 에이전트한테 **장기 기억**을 달아줍니다.

한 번 깔아두면, 매 세션에서 뭘 실패했고 어떻게 고쳤는지 알아서 기록합니다. 그리고 그 경험이 진짜 도움이 됐는지 강화학습으로 검증합니다. 다음 세션? 같은 실수하기 전에 검증된 해결법을 먼저 꽂아줍니다.

이런 식입니다: *"지난번에 이 에러 났을 때 async context manager로 해결했잖아 (Q:0.8)"* — 삽질하기 전에 먼저 알려줌.
...
**0.5 넘으면** Forge가 빠뜨리는 것보다 잡아주는 게 더 많다는 뜻.
**0.7 넘으면** 에이전트가 과거 세션 덕을 확실히 보고 있다는 뜻.
...
### 자동 학습
세션이 끝날 때마다 transcript를 분석해서 새로운 실패 패턴을 뽑아냅니다. 내가 뭔가 태깅하거나 기록할 필요 없이, 코딩만 하면 됩니다.

### 실시간 경고
코딩 도중 에러가 나면, 과거에 비슷한 에러가 있었는지 DB에서 즉시 매칭합니다. 있으면 바로 경고: "전에도 이런 적 있었고, 이렇게 해결했어."
...
### 프로젝트 간 전파
같은 패턴이 2개 이상 프로젝트에서 반복되면, 자동으로 글로벌 경험으로 올립니다. A 프로젝트에서 배운 게 B 프로젝트에서도 써먹어집니다.

### 서킷 브레이커
에이전트가 같은 실패를 계속 반복하고 있으면 자동 감지. 토큰 더 태우기 전에 끊어줍니다.

## 나한테 맞는 도구인가?

- Claude Code를 자주 쓴다 (주 3회 이상) → **맞음**
- 에이전트가 같은 실수를 반복하는 게 눈에 보인다 → **맞음**
- 매번 같은 걸 고쳐주는 게 지겹다 → **맞음**
- Cursor / Copilot 등 다른 도구를 쓴다 → **아직은 안 됨** (Claude Code 전용)
...
전부 선택사항. 기본값이 이미 최적화되어 있으니 안 건드려도 됩니다.

## 개인정보

데이터는 전부 `~/.forge/forge.db` 한 파일에 로컬 저장. 어디로도 안 나갑니다. 보고 싶으면 SQLite 뷰어로 열어보면 되고, 지우고 싶으면 파일 하나 지우면 끝.
