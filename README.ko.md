...
### 비교: 에이전트 실수를 지금 어떻게 처리하고 있나요?

| | 도구 없음 | CLAUDE.md만 | **Forge** |
|---|---|---|---|
| **에러 반복** | 매 세션 같은 에러 | 규칙 썼으면 줄어듦 | 발생 전에 자동 경고 |
| **지식 캡처** | 세션 끝나면 소실 | 직접 규칙 작성 | 매 세션 자동 캡처 |
| **관리 노력** | 없음 (대신 고통 누적) | 모든 규칙 직접 관리 | 제로 — 알아서 학습하고 잊음 |
| **프로젝트 간 공유** | 없음 | 복붙 | 2개+ 프로젝트에서 보이면 자동 승격 |
| **효과 측정** | 불가 | 불가 | **Forge Score** — 8개 지표 추적 |
| **시크릿 유출 방지** | 없음 | 없음 | 내장 감지 (API 키, 토큰) |
| **세션 실패 감지** | 수동 | 수동 | 실시간 패턴 매칭 |
| **컨텍스트 토큰 비용** | N/A | 정적 (항상 같은 크기) | 동적 — 검증된 것만 주입 |

### Forge + CLAUDE.md — 둘 다 쓰세요

해결하는 문제가 다릅니다:

| | Forge | CLAUDE.md |
|---|---|---|
| **내용** | 실제 세션에서 학습한 동적 패턴 | 직접 정의하는 정적 규칙 |
| **예시** | "이 async 패턴이 3세션 연속 타임아웃" | "DB에는 항상 `async with` 사용" |
| **업데이트** | 자동 (Q값 RL, 시간 감쇠) | 수동 (직접 편집) |
...
| | |
|---|---|
| **설치 시간** | ~30초 (`forge setup`) |
| **런타임 의존성** | 2개 (typer, pyyaml) — 무거운 프레임워크 없음 |
| **데이터 저장** | SQLite 파일 하나, 로컬 전용 |
| **추적하는 경험 유형** | 5가지 (실패, 결정, 규칙, 지식, 실험) |
| **세션당 추적 지표** | 8개 KPI (학습, 라우팅, 효율 등) |
| **경고 포맷** | 4종, A/B 테스트로 자동 선택 |
| **가드 hooks** | 4종 (시크릿 감지, no-verify 차단, compact 제안, 비용 추적) |
| **테스트** | 1,100+개 (전체 통과) |
